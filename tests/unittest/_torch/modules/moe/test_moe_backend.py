...
def test_megamoe_cutedsl_tuning_mode_forces_top_maxt_bucket(monkeypatch):
    ...
    moe = _make_megamoe_cutedsl_for_ctor_test(tactic_autotune=True)
...
def test_megamoe_cutedsl_tactic_autotune_defaults_off():
    ...
    # the flag the backend records NO profiling-scratch request, so the op's
    ...
    moe_on = _make_megamoe_cutedsl_for_ctor_test(tactic_autotune=True)
    assert moe_on.tactic_autotune is True


def test_megamoe_cutedsl_tactic_autotune_threading_and_validation():
    # ``tactic_autotune`` threads through create_moe_backend into the
    # MegaMoECuteDsl constructor (mirroring combine_format) and is rejected
    # for every other backend class.
    model_config = ModelConfig(
        mapping=Mapping(world_size=1, rank=0, tp_size=1, moe_tp_size=1, moe_ep_size=1),
        moe_backend=MoeBackendType.MEGAMOE_CUTEDSL.value,
        skip_create_weights_in_init=True,
    ...
    with pytest.raises(ValueError, match=r"tactic_autotune=True is only supported"):
        create_moe_backend(

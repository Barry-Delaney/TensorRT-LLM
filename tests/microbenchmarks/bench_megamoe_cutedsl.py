...
Perf knobs (passed explicitly into the backend / AutoTuner APIs; no env vars):
- ``--combine-format {bf16,fp8,fp4}`` -> ``create_moe(combine_format=...)``
  backend constructor argument (bf16 / 32e4m3xe8m0 / 16e2m1xbf16)
- ``--autotune``                      -> ``create_moe(tactic_autotune=True)``
...
def _bench_one(tpr: int, mapping: Mapping, args, dtype) -> dict:
    ...
    with create_moe(
        ...
        # Explicit MegaMoE tactic-sweep opt-in (default OFF): without it the
        # autotune() forward below would run the deterministic heuristic
        # tactic and silently report autotune=true. Rank-identical (same argv).
        tactic_autotune=bool(args.autotune),
    ) as moe:
...
def main():
    ...
    ap.add_argument(
        "--autotune",
        action="store_true",
        help="opt into the MegaMoE tactic sweep (create_moe(tactic_autotune="
        "True), default OFF) and run one forward inside autotune() (real "

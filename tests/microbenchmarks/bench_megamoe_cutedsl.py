...
Reproducible from the repo with NO external dependency: it drives the
``MEGAMOE_CUTEDSL`` backend through the same public ``create_moe`` -> ``forward``
path the committed unit test (tests/unittest/_torch/modules/moe/test_moe_module.py)
uses, with synthetic NVFP4 weights + uniform-random routing, and times the fused
dispatch + fc1 + fc2 + combine MoE forward.
...
# Repo-relative import of the committed MoE test helpers -- no external dependency.
# tests/microbenchmarks/bench_megamoe_cutedsl.py -> repo root is parents[2].
...
# ---- dist + config setup (adapted from test_moe_module.py) -------------------
...
def _mapping_dep(world_size: int) -> Mapping:
...
def _model_config(num_experts, hidden, intermediate, dtype, mapping, quant_config, max_num_tokens):
    pc = PretrainedConfig()
...
def _time_cuda_graph(fwd, args, ws: int) -> list:
    """Time the forward via CUDA-graph replay (kernel-bound).

    Capture ``--iters-per-graph`` forwards, ``dist.barrier()`` between replays to
    re-align EP ranks (outside the timed window). Warmup on a side stream so the
    cute.compile JIT finishes before capture.
    """
    ...
    for _ in range(args.iters):
        ...
        torch.cuda.synchronize()
        per_iter.append(start.elapsed_time(end) / ipg)  # per-forward
    return per_iter


def _bench_one(tpr: int, mapping: Mapping, args, dtype) -> dict:
    """Build, warm up, and time a fresh MEGAMOE_CUTEDSL backend.

    Sized for ``tpr`` per-rank tokens; times the MoE forward over ``args.iters``
    iterations after ``args.warmup`` warmup launches. Default is EAGER forward
    latency (includes host launch gaps -- not kernel-bound); ``--cuda-graph``
    switches to kernel-bound CUDA-graph timing (see the module docstring).
    """
    ...
    mcfg = _model_config(
        num_experts, hidden, intermediate, dtype, mapping, qconfig, max_num_tokens=tpr
    )
    ...
    del weights  # free the large expert weights; x / router_logits free on return
...
def main():
    ...
    mapping = _mapping_dep(world_size)
    mapping.rank = rank

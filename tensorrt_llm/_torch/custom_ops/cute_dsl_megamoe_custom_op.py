...
# In-tree MegaMoE kernel. The kernel is rank-independent (the peer mapper carries
# the PE id at runtime) and selects the cross-rank combine wire format via
# ``combine_format``; it is driven through the validated plain-arg AOT launch ABI.
...
# in-kernel barrier (counter, signal) has already been fenced-reset. The per-call
# runner is rebuilt every op call, so a per-runner flag would be None every call
# and the fence would fire EVERY launch -- a per-MoE-layer host sync + 2 NCCL
# barriers + zero (tanks decode TPOT ~10x, and the async zero clobbers peers'
# in-kernel signal writes -> the warmup hang). This module-global makes the fence
# fire ONCE per (workspace pair) as the design intends; steady-state launches
# skip it and ride the kernel's device-side self-reset. See the fence in forward().
...
if IS_MEGAMOE_OP_AVAILABLE:
    ...
    # would accumulate N x local_bytes and OOM (observed: ~36 x 15 GiB at
    # 16384 tokens / EP8 -> 258 GiB allocated, CUDA OOM). These profiling
    # workspaces are throwaway -- only the winning tactic is used at runtime --
    # so we free a candidate's workspace as soon as the next candidate is
    # allocated.
    ...
    def _get_or_alloc_local_workspace(
        ...
        # to ONE live local workspace instead of one-per-candidate, preventing
        # the high-``max_tokens_per_rank`` OOM. Correctness is unchanged: within
        # a single candidate's multi-launch profiling the key HITS the cache so
        # the same buffer is reused (the per-launch phase-preserving zero still
        # sees its own counter region), and a fresh candidate gets a fresh
        # counter=0 buffer exactly as before. Runtime (non-tuning) caching is
        # untouched -- the tuning-key set is empty outside tuning, so distinct
        # MoE shapes coexist normally.
    ...
    class MegaMoeSymmMemProvider:
        ...
        def __init__(
            ...
            *,
            ...
            # The kernel returns a unified (T, hidden) output via an INTERNAL top-k
            # reduce -> the symmetric combine region is (max_T, 1, hidden) and the op
            # aliases it as the 2D output_activation.
            self.combine_k = 1
    ...
    class Sm100MegaMoENvfp4Runner(TunableRunner):
        ...
        def get_valid_tactics(
            ...
            **kwargs,
            ...
            # for the UBLK store and verified numerically identical to non-bulk
            # (bulk-vs-non-bulk rel-RMSE 0 on B200 EP2), so fp8 bulk is left tunable
            # here. ``_unpack_tactic(t)[5]`` is ``use_bulk_fc2_store``.
        ...
        def _build_kernel(self, tactic: Tuple):
            ...
            # The kernel is rank-independent (the peer mapper carries the PE id at
            # runtime) -> NO ``local_rank``; ``combine_format`` selects the cross-rank
            # combine wire format.
            kernel_cls, CombineFormat = _import_megamoe_kernel()
        ...
        def forward(
            ...
            **kwargs,
            ...
            if tactic == -1 or tactic is None:
                ...
                # parity-verified, but this non-autotune default keeps ALL quantized
                # combine non-bulk as a deterministic fallback (matches the kernel
                # tester's BEST fp4/fp8 tactic: 256x256x256, cluster 2x1, standalone;
                # ~3-5.5% faster than reuse_dispatch at EP8). The autotuner
            ...
            # The kernel self-resets the leading counter prefix at its tail,
            # so SAME-tactic reuse needs NO per-launch clear -> steady-state E2E has
            # no full-workspace memset (the core perf win). BUT the autotuner sweeps
            # many tactics (layouts) reusing the ONE provider shared_workspace, so a
            # stale layout's counters would deadlock the next candidate's barrier.
            # Zero shared_workspace on each tactic CHANGE (first-use clear for the
            # new layout), mirroring the kernel-team tester's per-candidate
            # ``shared_ws.zero_()``. local_workspace is per-tactic-cached (fresh
            # ``torch.zeros``) so it needs no extra clear. Under CUDA graph the
            # eager pre-pass does this once; the captured/replayed launches skip it.
            # In-kernel NVLink-barrier phase coherence. The kernel's sense-reversing
            # dispatch barrier pairs a PERSISTED rank-local nvlink_barrier_counter
            # (kept OUTSIDE the device tail-reset prefix so it rides launches)
            # with a PERSISTED shared nvlink_barrier_signal. Two regimes:
            #
            #  * SAME tactic (steady state): preserve the counter and let the
            #    pair ride -> NO per-launch shared-signal zero (that would race a
            #    peer's in-kernel barrier write and deadlock).
            #  * TACTIC CHANGE: every AutoTuner candidate boundary AND the
            #    profiling->real-run / first-steady-state transition. The new
            #    tactic's per-tactic-cached local counter is fresh 0 while the
            #    shared signal still carries the PREVIOUS tactic's phase, so the
            #    pair is DECOUPLED and the next dispatch barrier spins forever
            #    (the autotuner candidate sweep deadlocked here). Reset BOTH the
            #    local counter and the 8-byte shared signal to a common phase 0 --
            #    the local is NOT always fresh here: at the profiling->real-run
            #    transition the winning tactic's local is cache-reused with an
            #    ALREADY-ADVANCED counter -- fenced by a cross-rank barrier so no
            #    peer reads the signal mid-reset. Fires ONCE per tactic (not per launch) and
            #    -- inside choose_one -- only on each candidate's first (warmup)
            #    launch, so it never lands in a timed window.
            #
            # NOTE: the fence uses the default (WORLD) PG, correct for the
            # pure-DEP bench (world == EP group). A general TP x EP deployment
            # must fence on the EP group (moe_ep_group_pg) instead.
            # The fence does a host sync + collective barrier, both ILLEGAL
            # during CUDA-graph capture. The bench (and production) always run
            # an EAGER pre-pass before capturing, and that pre-pass fires the
            # fence (tactic boundary) so the (counter, signal) pair is already
            # coherent by capture time; a captured forward uses ONE fixed tactic
            # (no boundary), so we safely skip the fence under capture and just
            # preserving-zero (a memset, capture-legal).
            _capturing = torch.cuda.is_current_stream_capturing()
            # Fire the (counter, signal) barrier-reset fence ONCE per
            # (local_ws, shared_ws, world) triple, tracked in the process-global
            # ``_MEGAMOE_FENCED_KEYS``. The runner is rebuilt every op call
            # (so a per-runner ``_last_fenced_tactic`` would be None every call
            # -> the fence would fire on EVERY launch: a per-layer host sync + 2
            # NCCL barriers + zero, which both badly hurts decode TPOT AND opens
            # the async-zero clobber race below). Keying on the workspace ptrs
            # makes it fire once per distinct (tactic local_ws, layout shared_ws)
            # -- correct for both the default fixed-tactic path and the autotune
            # candidate sweep (each candidate's distinct local_ws re-fences once
            # on its first/warmup launch). Steady-state launches then ride the
            # kernel's device-side tail self-reset. The fence does a host sync +
            # collective barrier, both ILLEGAL during CUDA-graph capture; the
            # eager warmup pre-pass fences each workspace once before capture, so
            # a captured forward (same workspace, already keyed) safely skips it.
            _fkey = (
            ...
            if (_fkey not in _MEGAMOE_FENCED_KEYS) and not _capturing:
                ...
                # subgroup is the general fix; tracked as a follow-up.)
                ...
                # barrier spins forever (the async-only hang that
                # CUDA_LAUNCH_BLOCKING=1 masked). Sync so the zeros LAND before the
            ...
            # else (SAME tactic, steady state): do NOTHING. The kernel
            # tail-resets its local counter prefix device-side every launch and
            # overwrites its scratch data regions, so a per-launch host memset of
            # the local workspace would be REDUNDANT and costly (a whole-workspace
            # memset is a large fraction of decode E2E). The tactic-change fence
            # above already gives the first launch of each tactic a clean
            # (counter, signal); the kernel rides from there.

            # === pure cute.compile (JIT) launch -- kernel-team mega_runner ABI ===
            ...
            # carries the peer mapper. cute.compile's built-in file cache + the
            # per-tactic ``kernel_cache`` handle reuse -- no AOT / disk round-trip.
            import cutlass

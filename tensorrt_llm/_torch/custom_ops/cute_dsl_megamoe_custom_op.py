...
* ``trtllm::cute_dsl_megamoe_nvfp4_blackwell`` is the registered torch
  ...
  ``run_moe``. ``tactic_autotune=True`` (bench-only) runs
...
if IS_MEGAMOE_OP_AVAILABLE:
    ...
    def cute_dsl_megamoe_nvfp4_blackwell(
        ...
        backend in ``mega_moe_cute_dsl.py``). ``tactic_autotune=True`` (bench
        opt-in) picks the tactic via AutoTuner per call; the default runs the
        deterministic token-bucket heuristic (``default_megamoe_tactic``).

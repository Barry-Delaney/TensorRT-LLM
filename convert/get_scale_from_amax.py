...
def load_and_preprocess_state_dict(modelopt_state_root, world_size=8):
    ...
    for rank in range(world_size):
        state_dict_list.append(
            torch.load(
                f"{modelopt_state_root}/amax_dict_rank{rank}-mp{world_size}.pt", map_location="cuda:0")
        )
...
def get_scales_from_amax(start_layer, end_layer, renamed_state_dict, scale_dir):
    ...
    for layer_idx in range(start_layer, end_layer):
        amax_keys_per_layer = [
            ...
            if (x.startswith(f'model.layers.{layer_idx}.mlp.experts.') and x.endswith(".input_quantizer._amax"))
        ]
        ...
        for k in amax_keys_per_layer:
            ...
            scales[f'model.layers.{layer_idx}.mlp.experts.{expert_idx}.w{weight_idx}.input_scale'] = val.unsqueeze(0) / 448

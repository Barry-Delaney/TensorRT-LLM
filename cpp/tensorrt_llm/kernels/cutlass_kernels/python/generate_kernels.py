...
def generate_sm90_mixed_type_grouped_gemm_operations(is_arch_enabled):
    ...
    cta_shapes_mn = list(product(M_TILES, N_TILES))
    # M_TILES = [64, 128]  # Currently M tile must be 128 for Grouped GEMM
    # N_TILES = [16, 32, 64, 128, 256]
    # cta_shapes_mn = list(product(M_TILES, N_TILES)) + [(256, 128), (256, 256)]
    ...
    for dtype_combo, quant_op, epi_tag, cta_shape_mn, cga_shape in partial_args:
        ...
        for mainloop_schedule in mainloop_schedules:
            if (cta_shape_mnk == (128, 128, 512) and mainloop_schedule
                    == KernelScheduleType.TmaWarpSpecializedCooperative):
                continue
...
if __name__ == "__main__":
    ...
    op_groups = dict()
    op_groups_mixed_dtype = dict()
    for op in operations:
        if should_skip(op):
            continue
        dict_key = (op.gemm_kind, op.arch, op.cta_shape[0])
        dic = op_groups_mixed_dtype if is_mixed_dtype(op) else op_groups
        op_group = dic.get(dict_key, list())
        op_group.append(op)
        dic[dict_key] = op_group
    ...
    for key, value in op_groups.items():
        ...
        file_counter += 1
    for key, value in op_groups_mixed_dtype.items():
        gemm_kind, _, _ = key
        out_file = os.path.join(
            output_dir, GemmKindNames[gemm_kind],
            f"cutlass_kernel_file_{file_counter}.generated.cu")
        write_file([moe_mixed_gemm_inl], value, out_file)
        file_counter += 1
